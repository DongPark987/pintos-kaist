...
/* 첫 번째 사용자 영역 프로그램 "initd"를 시작합니다. 이 프로그램은
 * FILE_NAME에서 로드됩니다. 새로운 스레드는 process_create_initd()가 반환되기
 * 전에 스케줄될 수 있습니다 (심지어 종료될 수도 있음). initd의 스레드 ID를
 * 반환하거나 스레드를 생성할 수 없는 경우 TID_ERROR를 반환합니다. 이 함수는 한
 * 번만 호출되어야 합니다. */
tid_t process_create_initd(const char *file_name) {
  ...
  tid = thread_create(strtok_r(file_name, " ", save_ptr), PRI_DEFAULT, initd,
                      fn_copy);
  if (tid == TID_ERROR)
...
/* 현재 프로세스를 'name'으로 복제합니다. 새로운 프로세스의 스레드 ID를
 * 반환하거나 스레드를 생성할 수 없는 경우 TID_ERROR를 반환합니다. */
...
/* 부모 프로세스의 주소 공간을 복제하려면 이 함수를 pml4_for_each에
 * 전달하십시오. 이 기능은 프로젝트 2 전용입니다. */
static bool duplicate_pte(uint64_t *pte, void *va, void *aux) {
  ...
  /* 3. TODO: 자식을 위해 새로운 PAL_USER 페이지를 할당하고 결과를 NEWPAGE에
   * 설정하세요. */
   ...
   *    TODO: 부모의 페이지가 쓰기 가능한지 확인하세요 (결과에 따라 WRITABLE을
   * 설정하세요). */
  memcpy(newpage, parent_page, PGSIZE);

  writable = is_writable(pte);

  /* 5. 주소 VA에 대해 자식의 페이지 테이블에 새 페이지를 WRITABLE 권한으로
   * 추가하세요. */
...
static void __do_fork(void *aux) {
  ...
  /* TODO: 부모의 인터럽트 플래그를 어떻게든 전달하세요. (예: process_fork()의
   * if_) */
  ...
  // 				tmp_fdt[parent->fdt[i].dup2_num] =
  // file_duplicate(parent->fdt[i].file);
  // 		}
  // 	}
  // }
...
error:
  ...
  trash = list_entry(&current->child_info->child_elem, struct child_info,
                     child_elem);
  free(trash);
  sema_up(&current->parent->fork_sema);
...
int process_exec(void *f_name) {
   ...
   * 이는 현재 스레드가 재스케줄될 때 실행 정보를 구조체 멤버에 저장하기
   * 때문입니다. */
 ...
 * 대해 process_wait()가 성공적으로 호출된 경우에도 즉시 -1을 반환하고 기다리지
 * 않습니다.
 *
...
int process_wait(tid_t child_tid UNUSED) {
  ...
  while (true) {
    find_child = -1;
    for (struct list_elem *cur = list_begin(&curr->child_list);
         cur != list_end(&curr->child_list); cur = list_next(cur)) {
...
static bool load(const char *file_name, struct intr_frame *if_) {
  ...
  for (token[args_cnt] = strtok_r(file_name, " ", &save_ptr);
       token[args_cnt] != NULL;
       token[args_cnt] = strtok_r(NULL, " ", &save_ptr)) {
    // printf("'%s'\n", token[args_cnt]);
...
static bool lazy_load_segment(struct page *page, void *aux) {
  ...
  /* TODO: 이 함수는 VA 주소에서 첫 번째 페이지 폴트가 발생했을 때
   * 호출됩니다. */
  ...
  if (file_read(file, kpage, read_bytes) != (int)read_bytes) {
    vm_dealloc_page(page); // 디얼록을 왜 여기서 하는지 모르겠어요~!~!~!
    return false;
  }
  // memset(kpage + read_bytes, 0, zero_bytes);

  /* Add the page to the process's address space. */
  // if (!pml4_set_page(curr->pml4, va, kpage, true)) {
  //   printf("fail\n");
  //   return false;
 ...
 * Return true if successful, false if a memory allocation error
 * or disk read error occurs. */
/* 주어진 파일(FILE)에서 오프셋(OFS) 위치에서 시작하는 세그먼트를 가상 주소
 * UPAGE에 로드합니다. 총 READ_BYTES + ZERO_BYTES 바이트의 가상 메모리를
 * 초기화합니다:
 *
 * - UPAGE에서 시작하는 READ_BYTES 바이트는 OFS 오프셋에서 파일로부터 읽어와야
 * 합니다.
 *
 * - UPAGE + READ_BYTES에서 시작하는 ZERO_BYTES 바이트는 0으로 초기화되어야
 * 합니다.
 *
 * 이 함수에 의해 초기화된 페이지는 WRITABLE이 true인 경우 사용자 프로세스에서
 * 쓰기 가능하며, 그렇지 않으면 읽기 전용입니다.
 *
...
static bool setup_stack(struct intr_frame *if_) {
  ...
  /* TODO: Your code goes here */
  /* TODO: 스택을 stack_bottom에 매핑하고 즉시 페이지를 할당합니다.
