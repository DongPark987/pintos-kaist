...
/* 해시 테이블 H를 초기화하여 해시 함수 HASH를 사용하여 해시 값을 계산하고,
   보조 데이터 AUX를 사용하여 해시 요소를 비교합니다. */
/* hash_init() 내부에서 hash의 멤버를 malloc()으로 할당하는데, 이때 malloc()이
   실패하는 경우 hash_init()은 false를 반환합니다. */
bool hash_init(struct hash *h, hash_hash_func *hash, hash_less_func *less,
               void *aux) {
  h->elem_cnt = 0;   // 요소 수 초기화
  h->bucket_cnt = 4; // 버킷 수 초기화
  // 버킷 배열 동적 할당
  ...
  // 동적 할당이 성공한 경우
  if (h->buckets != NULL) {
    hash_clear(h, NULL); // 해시 테이블 초기화 (해시 테이블을 비운다)
    return true;         // 초기화 성공
  } else
    return false; // 동적 할당 실패로 초기화 실패
}
...
널 포인터를 반환합니다. 이미 동일한 요소가 테이블에 존재하는 경우,
NEW를 삽입하지 않고 해당 요소를 반환합니다. */
struct hash_elem *hash_insert(struct hash *h, struct hash_elem *new) {
  // 1. 새 요소 NEW를 삽입할 버킷을 찾아옵니다.
  struct list *bucket = find_bucket(h, new);
  // 2. 같은 값을 가지는 요소를 버킷에서 찾아옵니다.
  struct hash_elem *old = find_elem(h, bucket, new);
  // 3. 이미 같은 값의 요소가 테이블에 없으면
  // 4. 새 요소 NEW를 해당 버킷에 삽입합니다.
  if (old == NULL)
    insert_elem(h, bucket, new);
  // 5. 해시 테이블을 재조직합니다.
  rehash(h);
  // 6. 이미 같은 값의 요소가 존재하면 해당 요소를 반환합니다.
  return old;
}
...
/* 해시 테이블 H에서 요소 E와 동등한 요소를 찾아 반환하거나,
   테이블에 동등한 요소가 없을 경우 null 포인터를 반환합니다. */
